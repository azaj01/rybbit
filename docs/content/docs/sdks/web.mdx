...
| `enableRemoteConfig`   | `boolean` | `true`  | Enable fetching configuration from your Rybbit instance on init. Disable to skip remote config fetch.                                                                  |
| `remoteConfigTimeout`  | `number` | `3000`  | Timeout in milliseconds for fetching remote configuration.                                                                                                              |
| `replayBufferSize`     | `number` | `250`   | Number of session replay events to buffer before sending to server.                                                                                                     |
| `replayBatchInterval`  | `number` | `5000`  | Interval in milliseconds for sending session replay event batches.                                                                                                      |
...
```typescript
rybbit.captureError(error: Error | ErrorEvent, context?: TrackProperties);
```
...
<Callout type="info">
Manual error capture only works when error tracking is enabled in your dashboard settings. If it's disabled, calling `captureError()` will log a warning.
</Callout>
